...
class UsuarioController extends Controller
{
    ...
    public function actualizarPerfil(Request $request)
    {
        ...
        if ($user) {
            ...
            $user->USU_FEC_MODIFICACION = now()->format('Y-m-d H:i:s');

            $user->USU_SEXO = $request->input('P_GENERO');
            if($request->input('P_FECHA_NACIMIENTO')){
                $user->USU_FEC_NACE = $request->input('P_FECHA_NACIMIENTO');
            }
        }
        try {
            $user->save();
            return redirect('/usuario/perfil')->with('success', 'Los datos fueron actualizados con exito');
            // return response()->json(['tipo' => 'success', 'mensaje' => 'Los datos fueron actualizados con exito']);
        } catch (\Throwable $th) {
            return redirect('/usuario/perfil')->with('error', 'Ocurrio un problema intentalo otra vez');
